...
trait SMGMonInternalState extends SMGMonState {
  ...
  protected def processAlertsAndLogs(prevStateWasInherited: Boolean): Unit = {
    ...
    lazy val wasHardError = prevStates.lengthCompare(maxHardErrorCount) >=0 &&
      prevStates.take(maxHardErrorCount).forall(!_.isOk)
    lazy val isHardError = !curState.isOk &&
      prevStates.lengthCompare(maxHardErrorCount - 1) >=0 &&
      prevStates.take(maxHardErrorCount - 1).forall(!_.isOk)
        ...
        if (isHardError) {
          if (!isSilencedOrAcked)
            ...
            log.warn(s"SMGMonInternalState.addState: " +
              s"skipping notifications to ${notifCmds.map(_.id).mkString(",")} for " +
              s"silenced($isSilenced) or acked($isAcked) state: $id")
        }
