  ...
  implicit val smgFetchCommandReads: Reads[SMGFetchCommand] = {
    (
      (JsPath \ "id").read[String].map(id => prefixedId(id)) and
        ...
        (JsPath \ "ignd").readNullable[String].map(_.getOrElse("false") == "true") and
...
object SMGRemoteClient {
  ...
  implicit val smgFetchCommandWrites: Writes[SMGFetchCommand] = new Writes[SMGFetchCommand] {
    def writes(fc: SMGFetchCommand): JsValue = {
      ...
      if (fc.commandDesc.isDefined) mm += ("desc" -> Json.toJson(fc.commandDesc.get))
      ...
      if (fc.ignorePassedData)  mm += ("ignd" -> Json.toJson("true"))
