...
class MonitorSpec extends PlaySpecification with MockitoSugar {
  ...
  "SMGMonitor.receiveObjMsg" should {
    "work and not send messages and logs on OK state" in {
      ...
      ms.head.currentState.ts shouldEqual startOfTest + 120
      ms.head.currentState.desc shouldEqual "OK: value=1.5 : ( 3.0 / 5.0 )"
      ms(1).isOk shouldEqual true
      ms(1).currentState.ts shouldEqual startOfTest + 120
      ms(1).currentState.desc shouldEqual "OK: value=2.5"
    }
    ...
    "send a single alert on hard error and proper number of mon log msgs" in {
      ...
      ms.head.currentState.ts shouldEqual startOfTest + 420
      ms.head.currentState.desc shouldEqual "CRIT: 6.0 >= 5.0 : ( 3.0 / 5.0 )"
      ms(1).isOk shouldEqual true
      ms(1).currentState.ts shouldEqual startOfTest + 420
      ms(1).currentState.desc shouldEqual "OK: value=1.0"
    ...
    "send two alerts on hard error with a state change" in {
      ...
      ms.size shouldEqual 2
      ms.head.isOk shouldEqual false
      ms.head.currentState.ts shouldEqual startOfTest + 480
      ms.head.currentState.desc shouldEqual "CRIT: 6.0 >= 5.0 : ( 3.0 / 5.0 )"
      ms(1).isOk shouldEqual true
      ms(1).currentState.ts shouldEqual startOfTest + 480
      ms(1).currentState.desc shouldEqual "OK: value=1.0"
    ...
    "work with fetch errors" in {
      ...
      ms.size shouldEqual 2
      ms.head.isOk shouldEqual false
      ms.head.currentState.ts shouldEqual startOfTest + 360
      ms.head.currentState.desc shouldEqual "Fetch error: exit=7, OUTPUT: fetch error"
      ms(1).isOk shouldEqual false
      ms(1).currentState.ts shouldEqual startOfTest + 360
      ms(1).currentState.desc shouldEqual "Fetch error: exit=7, OUTPUT: fetch error"
    }
    ...
    "work with mix of var errors and fetch errors" in {
      ...
      ms.size shouldEqual 2
      ms.head.isOk shouldEqual false
      ms.head.currentState.ts shouldEqual startOfTest + 300
      ms.head.currentState.desc shouldEqual "Fetch error: exit=7, OUTPUT: fetch error"
      ms(1).isOk shouldEqual false
      ms(1).currentState.ts shouldEqual startOfTest + 300
      ms(1).currentState.desc shouldEqual "Fetch error: exit=7, OUTPUT: fetch error"
    }
  ...
  "SMGMonitor.processPfMsg" should {
    "work and not send messages and logs on OK states" in {
      ...
      mon.receivePfMsg(SMGDFPfMsg(startOfTest + 60, pf.id, pfObjs, 0, List()))
      pfObjs.foreach { pfo =>
        mon.receiveObjMsg(SMGDFObjMsg(startOfTest + 60, pfo, List(2.0, 1.0), 0, List()))
      }

      mon.receivePfMsg(SMGDFPfMsg(startOfTest + 120, pf.id, pfObjs, 0, List()))
      pfObjs.foreach { pfo =>
      ...
      ms.head.isOk shouldEqual true
      ms.head.currentState.ts shouldEqual startOfTest + 120
      ms.head.currentState.desc shouldEqual "OK: value=1.0 : ( 3.0 / 5.0 )"
      ms(1).isOk shouldEqual true
      ms(1).currentState.ts shouldEqual startOfTest + 120
      ms(1).currentState.desc shouldEqual "OK: value=2.0"
    }
    ...
    "work and send messages and logs on pf error states" in {
      ...
      ms.size shouldEqual 2
      ms.head.isOk shouldEqual false
      ms.head.currentState.ts shouldEqual curTs
      ms.head.currentState.desc shouldEqual "Fetch error: exit=7, OUTPUT: pf error"
      ms(1).isOk shouldEqual false
      ms(1).currentState.ts shouldEqual curTs
      ms(1).currentState.desc shouldEqual "Fetch error: exit=7, OUTPUT: pf error"
    }
